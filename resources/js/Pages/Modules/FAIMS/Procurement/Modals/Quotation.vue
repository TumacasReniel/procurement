    ...
    >
        ...
        <template v-slot:footer>
            ...
            <b-button
                @click="submit()"
                variant="primary"
                :disabled="form.processing || !form.supply_officer_id"
                block
                >Save</b-button
            >
        </template>
...
export default {
    ...
    computed: {
        filteredSuppliers() {
            ...
            const rfq_supplier_ids = this.rfqs.map((r) => r.supplier?.id);

            // Merge exclusions
            const exclude_ids = new Set([...selected_ids, ...rfq_supplier_ids]);
