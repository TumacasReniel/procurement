  ...
  >
    ...
    <div class="text-center px-5 pt-2">
      <div class="mt-2">
        ...
        <p v-if="$page.props.flash.info" class="text-muted fs-12">
...
export default {
  ...
  provide() {
    return {
      mentionNotificationCenter: {
        ...
        resolveContextLabel: (notification) =>
          this.resolveMentionContextLabel(notification),
  ...
  methods: {
    ...
    hydrateMentionNotificationsFromProps(feed) {
      ...
      this.mentionNotificationState.items = Array.isArray(feed.data) ? feed.data : [];
    ...
    fetchMentionNotifications() {
      ...
      axios
        .get("/faims/procurement-mention-notifications", {
          params: {
            limit: 6,
          },
        })
    ...
    markMentionNotificationRead(notificationId) {
      return axios.patch(
        `/faims/procurement-mention-notifications/${notificationId}/read`
      );
    },
    dismissMentionNotification(notificationId) {
      this.markMentionNotificationRead(notificationId).finally(() => {
        this.fetchMentionNotifications();
      });
    },
    openMentionNotification(notification) {
      ...
      this.markMentionNotificationRead(notification.id).finally(() => {
        this.fetchMentionNotifications();
